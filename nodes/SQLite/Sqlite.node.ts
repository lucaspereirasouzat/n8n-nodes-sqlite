			...
			switch (operation) {
				...
				case 'insert':
					try {
						...
						const insertSQL = `INSERT INTO ${table}(${columnString}) VALUES ${items
							.map((item) => insertPlaceholder)
							.join(',')};`;
						const queryItems = insertItems.reduce(
							(collection: any, item: any) => collection.concat(Object.values(item as any)),
							[],
				...
				case 'update':
					try {
						...
						const queryQueue = updateItems.map((item: any) =>
							db.run(updateSQL, Object.values(item).concat(item[updateKey])),
						);
